...
if (isset($_POST['azione'])) {
	if ($_POST['azione']=='visualizza') {
		...
		try {
			...
			if ($result!=1) {
				echo json_encode(['esito'=>false, 'errore' => $result]);
			}
			echo json_encode(['esito'=>true, 'messaggio' => 'Cartella Clinica modificata con successo.']);
		} catch (Exception $e) {
		...
		try {
			...
			if ($result==1) {
				echo json_encode(['esito'=>true, 'messaggio'=>'Cartella Clinica aggiunta con successo.']);
			} else {
				echo json_encode(['esito'=>false, 'errore'=>$result]);
			}
		} catch (Exception $e) {
...
} elseif (isset($_SESSION['paziente'], $_SESSION['professionista'])) {
